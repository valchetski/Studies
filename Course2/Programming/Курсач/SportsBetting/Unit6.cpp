...
void TForm6::OpenStatistic()
{
        ...
        for (int i = 0; i < size; i++)
        {
                ...
                {
		        getline(file, str, '\t');
                        string temp;     //объ€вили переменную temp т.к. в следующей строчке
                        getline(file, temp, '\n');//str изменилс€ бы. а он нам дальше нужен
                        if (str.empty())
                        {
                                continue;
                        }
...
void TForm6::CalculateStatistic(TDate date1, TDate date2)
{
        ...
        Label10->Caption = "— " + DateToStr(date1) + " по " + DateToStr(date2);
}
