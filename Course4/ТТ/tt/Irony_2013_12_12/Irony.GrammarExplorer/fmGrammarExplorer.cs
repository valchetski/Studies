...
namespace Irony.GrammarExplorer {
  ...
  public partial class fmGrammarExplorer : Form {
    ...
    bool _treeClickDisabled; //to temporarily disable tree click when we locate the node programmatically
      ...
      private string il = @"IL_0000:  nop         
IL_0001: ldc.r8    
IL_000A: stloc.0    
IL_000B: ldc.i4.0
IL_000C: stloc.1    
IL_000D: br.s IL_0040
IL_000F: nop
IL_0010: ldloc.1     
IL_0011: ldc.i4.5
IL_0012: clt
...
IL_0050:  ret ";
    ...
    private void ShowCompilerErrors() {
      gridCompileErrors.Rows.Clear();
            for (int i = 0; i < Parser.list.Count; i+=2)
            {
                gridCompileErrors.Rows.Add(Parser.list[i+1], Parser.list[i], "");
                okparse = false;
            }
    ...
    private void ParseSample() {
      ...
      //} finally {
        ...
        ShowParseTree();
        ShowAstTree();
                using (var file = new FileStream("132.txt", FileMode.Create))
                {
    ...
    private void chkDisableHili_CheckedChanged(object sender, EventArgs e) {
      ...
      EnableHighlighter(!chkDisableHili.Checked);
        ...
        {
            Parser.oldToken = null;
            Parser.d = new Dictionary<string, string>();
            Parser.isOK = true;
            Parser.list = new List<string>();
            ParseSample();
            
            
            if (okparse && Parser.isOK)
            {
                richTextBox1.Text = il;
                string text = txtSource.Text;
                CSharpCompiler.CSharpCompilerMini.Begin(text);
            }
        }
    }//class
}