...
ставятся,  но  обязательно  ставятся  после  этих  знаков.  Расставьте  недостающие 
пробелы. Подсчитать количество исправлений */

declare
  text varchar(6203) := 'Притчи.и рассказы  с неожиданными финалами и мудростью, спрятанной  глубоко внутри, впечатляют намного больше, нежели просто умные мысли и цитаты. 
...
Мораль  :  вещи сами по себе не бывают плохими или хорошими. Их делает такими наше отношение.';
  noSpacesAfterSymbolsRegExp varchar(15) := '[.,!?:;][^ \n]';
begin
  DBMS_OUTPUT.put_line('Количество исправлений: ' || REGEXP_COUNT(text, noSpacesAfterSymbolsRegExp));
  ...
  --DBMS_OUTPUT.put_line(REGEXP_REPLACE(text, noSpacesAfterSymbolsRegExp, '\1 '));
