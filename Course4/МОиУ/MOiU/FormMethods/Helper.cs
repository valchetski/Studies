    ...
    public static class Helper
    {
        ...
        public static T[,] GetMatrix<T>(DataGridView dataGridView)
        {
            ...
            for (int i = 0; i < m.GetLength(0); i++)
            {
                ...
                {
                    m[i, j] = (T)Convert.ChangeType(dataGridView[j, i].Value, typeof(T));
                }
            }
            return m;
        }
        ...
        {
            int currentRowCount = dataGridView.RowCount;
            if (dataGridView.ColumnCount > 0)
            {
                if (rowCount > currentRowCount)
                {
                    for (int i = 0; i < rowCount - currentRowCount; i++)
                    {
                        ...
                        int valuesCount = dataGridView.ColumnCount;
        ...
        {
            int currentColumnCount = dataGridView.ColumnCount;
            if (dataGridView.ColumnCount > 0)
            {
                if (columnCount > currentColumnCount)
                {
                    for (int i = 0; i < columnCount - currentColumnCount; i++)
                    {
                        dataGridView.Columns.Add("", "");

                        int valuesCount = dataGridView.RowCount;
                        var randomValues = GetRandomValues<T>(valuesCount);
                ...
                {
                    for (int i = 0; i < currentColumnCount - columnCount; i++)
                    {
                        dataGridView.Rows.RemoveAt(dataGridView.RowCount - 1);
                    }
                }
        ...
        public static string ToPrint(DenseMatrix matrix)
        {
            string toPrint = "";
        ...
        public static string ToPrint(DenseVector vector)
        {
            string toPrint = "";
            for (int i = 0; i < vector.Count; i++)
            {
                toPrint += Math.Round(Convert.ToDecimal(vector[i]), 4) + " ";
            }
            toPrint += "\n";
            return toPrint;
        }
