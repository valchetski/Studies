...
procedure TForm1.BitBtn3Click(Sender: TObject); //линейный поиск
var i:integer; num:integer;
begin
  Memo1.Clear;  num:=1;
  k:=StrToInt(Edit5.Text);
  SaS.PLineSearch(k);
  if sas.s=[] then memo1.Lines.Add('Поиск не дал результатов') else begin
    for i:=1 to sas.num do
      if i in sas.s then begin
        with sas.MSport[i] do
          Memo1.Lines.Add(inttostr(num)+')'+'страна: '+country+', команда: '
          +team+', фамилия: '+fio+', номер: '+IntToStr(number)+', возраст '
  ...
  with sas.MSport[SaS.PDSearch(a1)] do begin
    if number<>0 then begin
      ...
      Memo1.Lines.Add(IntToStr(num)+')страна: '+country+', команда: '+team+
        ', фамилия: '+fio+', номер: '+IntToStr(number)+', возраст: '+IntToStr(age)+
        ', рост: '+IntToStr(growth)+', вес:'+IntToStr(weight));
    end else Memo1.Lines.Add('Поиск не дал результатов');
  end;
end;

procedure TForm1.N2Click(Sender: TObject); //создание
begin
...
procedure TForm1.N4Click(Sender: TObject);//сохранение
begin
