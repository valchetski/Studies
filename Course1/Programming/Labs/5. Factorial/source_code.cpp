...
typedef struct item //typedef вводит новое им€ дл€ типа
...
{ 
	...
	while (!scanf("%d", &n))
	{
		printf("ќшибка! ¬водить только целые числа!\n¬ведите n=");
		fflush(stdin);
	}
...
MNumber CreateMNumber(char initStr[]) //создает число
{ 
	MNumber number = {NULL, NULL, 0}; //число обнул€етс€
...
{ 
    ...
    p->digit = digit; //добавл€ем первую цифру
    p->next = p->prev = NULL; //цифра одна, поэтому указатели на след. и пред. элемент равны NULL
    if (number->head == NULL) //если один элемент в списке, то выполн€етс€
...
{ 
	...
	{
		if (p_factor)//начинаем с головы списка, если не пуста, то извлекаем значение
		{
		...
		for (int i = 1; i<=zeros; i++)//добавл€ютс€ нули к числу
			AddDigit(&multi, 0);
		zeros++;
		while (p_result) 
		{ 
			...
			{ 
				...
				value_result = 0; 
			digit = (value_result * value_factor + pos) % 10; //находим число единиц
			pos = (value_result * value_factor + pos) / 10; //находим число дес€тков
